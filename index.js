...
async function get_item(champion, position, browser) {
    ...
    let startItemLists = await startItemHandle.$$('.champion-stats__list');
    let startItem = [];
    for (let startItemList of startItemLists) {
        let arr = await startItemList.$$eval('.champion-stats__list__item img', el => el.map(el => /\/item\/(.*)\.png/.exec(el.src)).map(el => el[1]));
        startItem.push(arr);
    }

    let mainItemLists = await mainItemHandle.$$('.champion-stats__list');
    let mainItem = [];
    for (let mainItemList of mainItemLists) {
        let arr = await mainItemList.$$eval('.champion-stats__list__item img', el => el.map(el => /\/item\/(.*)\.png/.exec(el.src)).map(el => el[1]));
        mainItem.push(arr);
    }
